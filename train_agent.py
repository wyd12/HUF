...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, act_dim, gamma=0.99, hidden_sizes=[512, 256]):  # 400，4
...
def main():
    ...
    parser.add_argument('--gpu', type=str, default='0', help='gpu device.')                             #指定cpu运行，默认0
    parser.add_argument('--epochs', type=int, default=50, help='Max number of epochs.')                 #训练迭代次数，默认50
    parser.add_argument('--batch_size', type=int, default=32, help='batch size.')                       #批处理大小，默认32
    parser.add_argument('--lr', type=float, default=1e-4, help='learning rate.')                        #学习速率，默认0.0001
    parser.add_argument('--max_acts', type=int, default=250, help='Max number of actions.')             #最大动作数量，默认250
    parser.add_argument('--max_path_len', type=int, default=3, help='Max path length.')                 #最大路径长度，默认3
    parser.add_argument('--gamma', type=float, default=0.99, help='reward discount factor.')            #奖励折扣因子，默认0.99
    parser.add_argument('--ent_weight', type=float, default=1e-3, help='weight factor for entropy loss')#熵损失权重，默认0.0001
    parser.add_argument('--act_dropout', type=float, default=0.5, help='action dropout rate.')          #动作丢失率，默认0.5
    parser.add_argument('--state_history', type=int, default=1, help='state history length')            #状态历史长度，默认1
