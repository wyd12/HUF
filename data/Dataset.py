...
class DataSet(object):
    ...
    def load_kg(self, name,num_items):
        lines = open(name, 'r').readlines()
        embedding_map = []
        for index, l in enumerate(lines):
            tmps = l.strip()
            # inters = [i for i in tmps.split(' ')]
            # u_id, features = int(inters[0]), inters[1:]
            features = tmps.split(' ')
            features = features[1:]
            # u_id =index-1
            # features=inters[1:]
        ...
        feature_matrix = np.concatenate(embedding_map)
        zero_feature = np.zeros((1,50))
        #feature_matrix = feature_matrix[:num_items,:]

        feature_matrix = np.concatenate((zero_feature,feature_matrix))
    ...
    def split_data_sequentially(self, user_records, test_radio=0.3):
        train_set = []
        test_set = []

        for item_list in user_records:
            len_list = len(item_list)
            num_test_samples = int(len_list * test_radio)
            # num_test_samples =1
            train_sample = []
            test_sample = []
            # if len_list<3:
