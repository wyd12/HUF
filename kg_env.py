...
class BatchKGEnvironment(object):
    ...
    def _get_actions(self, path, done):
        """Compute actions for current node. 计算当前节点的行动"""
    ...
    def batch_action_mask(self, dropout=0.0, masked_user_num=[], step=0):
        ...
        for actions in self._batch_curr_actions:
            ...
            act_mask[act_idxs] = 1
            if user_num in masked_user_num:
                if step == 3:
                    for i in range(len(act_mask)):
                        if act_mask[i] == 1:
                            _, curr_node_type, curr_node_id = self._batch_path[user_num][-1]
                            relation, next_node_id = actions[i]
                            if relation == SELF_LOOP:
                                next_node_type = curr_node_type
                            else:
                                next_node_type = KG_RELATION[curr_node_type][relation]
